...
10. [실험 결과 — 3B Base 평가](#10-실험-결과--3b-base-평가)
11. [실행 방법](#11-실행-방법)
12. [로드맵](#12-로드맵)
13. [참고 문서](#13-참고-문서)
...
| PPL (통합 검증셋) | 5.709 |
| PPL (C4 Korean) | 5.717 |
| belebele_kor_Hang | 0.2189 (≈ 랜덤, base 정상) |
| global_mmlu_full_ko | 0.2339 (≈ 랜덤, base 정상) |
| Greedy 반복률 | 71.1% (SFT 후 개선 예정) |
| temp=1.0 반복률 | 9.3% |
...
| 검증셋 | PPL | BPT |
|--------|-----|-----|
| 3b_val (통합) | 5.709 | 2.513 |
| korean_c4 | 5.717 | 2.515 |
| korean_namuwiki | 25.881 | 4.694 |
| korean_wiki | 11.836 | 3.565 |

### 벤치마크 (lm-evaluation-harness)

| 벤치마크 | 3B Base | 랜덤 기준 |
|----------|---------|-----------|
| belebele_kor_Hang | 0.2189 | 0.25 |
| global_mmlu_full_ko | 0.2339 | 0.25 |

> Base model은 instruction-following 없이 4지선다 형식 벤치마크를 풀도록 최적화되지 않음. SFT 후 향상 기대.

### 생성 품질 (3-gram 반복률)

| 온도 | 3-gram 반복률 | EOS 종료율 |
|------|--------------|-----------|
| 0.0 (greedy) | 71.1% | 0% |
| 0.7 | 40.5% | 0% |
| 1.0 | 9.3% | 0% |

`no_repeat_ngram_size=3` 적용 시 반복률 0% 달성. EOS 종료율 0%는 base model 정상 특성.

### 권장 추론 파라미터 (base 실험용)

```python
temp=0.9, top_p=0.9, no_repeat_ngram=3, repetition_penalty=1.1
```

### SFT 진행 판단

**결론: SFT 진행 ✅** — loss 1.466 건강한 완료 시그널, 구조 문제 없음.

상세 보고서: `reports/2026-03-05_3B_BASE_EVALUATION_REPORT.md`
...
| 3B Base 평가 보고서 | `reports/2026-03-05_3B_BASE_EVALUATION_REPORT.md` | PPL/벤치마크/반복률 종합 평가 |
| PPL 평가 보고서 | `reports/2026-03-05_PPL_EVALUATION.md` | 4개 검증셋 PPL 상세 |
| 벤치마크 결과 | `reports/2026-03-05_BENCHMARK_RESULTS.md` | belebele, MMLU 상세 |
| 생성 품질 분석 | `reports/2026-03-05_GENERATION_QUALITY.md` | 반복률, 디코딩 파라미터 |
...
Phase 1은 완료됐고, 평가도 끝났다. PPL 5.709, 벤치마크는 base model답게 랜덤 수준, greedy 반복률 71%—솔직한 숫자다. 하지만 이 숫자들은 SFT가 해결해야 할 영역이지, 모델 구조의 문제가 아니다. 지금은 Phase 2 SFT를 준비하고 있다.
