...
![Phase 2](https://img.shields.io/badge/Phase_2_SFT-6%25_진행중-blue)
...
![SFT Data](https://img.shields.io/badge/SFT_Data-2.44M_samples-yellow)
![Status](https://img.shields.io/badge/Status-SFT_Training-ff9900)
...
| **Phase 2: SFT** | **🔄 진행 중** | **2,000/33,000 steps (6%), val_loss 1.956** |
...
| Train loss | 2.053 |
| **Val loss (best)** | **1.956** (단조 감소 중) |
| VRAM 사용 | **24.2GB** / 183GB per GPU (13.2%) |
| Grad norm | 1.0 (안정) |
| 예상 완료 | 2026-03-06 04:30경 (~5시간 잔여) |
| Base 모델 | checkpoint-0057000 (pretrain loss 1.466) |
| SFT 데이터 | **2,439,397 samples** (24개 소스, 7.48 GB) |
| 배치 크기 | bs=2, accum=4 (유효 배치 64) |

**SFT Val Loss 추이**:
```
Step  500: 2.073
Step 1000: 2.004  (-0.069)
Step 1500: 1.975  (-0.029)
Step 2000: 1.956  (-0.019)  ← 현재 BEST
```

> Catastrophic forgetting 없이 안정 수렴 중. NEFTune alpha=5.0 적용.
> 상세: `reports/2026-03-05_3B_SFT_PROGRESS_REPORT.md`
...
│   ├── korean_3b_sft.yaml             # 3B SFT 설정 (현재 사용 중)
...
**현재 SFT 진행 상황**:
- Step: **2,000 / 33,000** (6.06%)
- Val loss: **1.956** (단조 감소, catastrophic forgetting 없음)
- 데이터: 24개 소스 → **2,439,397 samples** (filter_sft_v2.py 5단계 필터)
- VRAM: 24.2 GB (13.2%), NEFTune alpha=5.0
...
| **Phase 2 (SFT)** | **🔄 진행 중** | **2K/33K steps, val_loss 1.956, 2.44M 샘플** |
| SFT 완료 후 벤치마크 | 📋 대기 | KoBEST, MMLU-KO, 반복률, EOS 종료율 |
| Phase 3 (ORPO) 조건부 판정 | 📋 대기 | 반복률 >5%일 경우 실행 |
...
| **SFT 학습 보고서** | `reports/2026-03-05_3B_SFT_PROGRESS_REPORT.md` | **Phase 2 SFT 전체 기록 (최신)** |
...
Phase 1은 완료됐고, v2 종합 평가도 끝났다. PPL 5.23, KoBEST 평균 43.69%, MMLU-KO 22.75%, Calibration Top-1 68.75%. 참고 모델(Llama-3.2-3B, Qwen2.5-3B) 대비 격차가 있지만, 41.12B 토큰과 63시간이라는 제한된 자원을 감안하면 납득할 수 있는 출발점이다.

지금은 **Phase 2 SFT가 진행 중**이다. 24개 소스에서 2,439,397개의 instruction-response 쌍을 모으고, 5단계 품질 필터를 거쳐 학습하고 있다. 1B에서의 5가지 실패(label leakage, EOS 절단, static padding, 검증 미비, 데이터 노이즈)를 모두 반영했다. Val loss는 2.073 → 1.956으로 일관되게 하락 중이며, catastrophic forgetting 없이 안정 수렴하고 있다. VRAM은 24.2 GB(13.2%)로 매우 여유 있다.

SFT가 완료되면 핵심 질문에 답할 차례다: **3B의 greedy 반복률이 5% 미만으로 떨어지는가?** 이것이 ORPO 없이 바로 배포할 수 있는지를 결정한다.
